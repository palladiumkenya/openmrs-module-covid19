 ...
 */
public class EligibleForCovidTestCalculation extends AbstractPatientCalculation implements PatientFlagCalculation {
	...
	public String getFlagMessage() {
		return "Eligible for COVID-19 test";
	}
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		Integer yesAnswer = 1065;
		Integer eligibilityQuestion = 165087;
		...
		for (int ptId : cohort) {
			...
			if (alive.contains(ptId) && lastCovidEncounter != null) {
				if (lastCovidEncounter.getEncounterType().getUuid().equals(CovidMetadata._EncounterType.COVID_SCREENING)) {
					eligible = EmrUtils.encounterThatPassCodedAnswer(lastCovidEncounter, cs.getConcept(eligibilityQuestion),
					    cs.getConcept(yesAnswer));
				}
			}
