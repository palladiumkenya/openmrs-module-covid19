...
/**
 * Calculates fully vaccination status Retired flag
 */
public class FullyVaccinatedCalculation extends AbstractPatientCalculation {
	 ...
	 */
	protected static final Log log = LogFactory.getLog(FullyVaccinatedCalculation.class);
	...
	public CalculationResultMap evaluate(Collection<Integer> cohort, Map<String, Object> parameterValues,
		...
		Integer FullyVaccinationStatus = 5585;
			...
			if (lastCovidAssessmentEncounter != null) {
				...
				for (Obs obs : lastCovidAssessmentEncounter.getObs()) {
					if (obs.getConcept().getConceptId().equals(CompletionStatusQuestion)
					        && (obs.getValueCoded().getConceptId().equals(FullyVaccinationStatus))) {
						fullyVaccinated = true;
					}
					
					if (inHivProgram.contains(ptId) && fullyVaccinated) {
						eligible = true;
					}
